...
> **최신성 재확인 (2026-09-14)**: 둘 다 여전히 활발히 유지보수 중입니다.
> `mcp-clickhouse`는 최신 MCP 프로토콜(2026-07-28 스펙) 지원과 `chdb` 연동
> 옵션이 추가됐고, `altinity-mcp`는 최신 태그 `v1.3.4`까지 나왔으며 OAuth
> 지원을 위한 별도 서브프로젝트(`go-mcp-oauth-sdk`)가 파생될 정도로 개발이
> 이어지고 있습니다. 아래 표/검증 내용의 방향성은 그대로 유효합니다.
