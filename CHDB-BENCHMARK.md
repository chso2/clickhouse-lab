...
> **2026-09-14 추가 재확인**: `chdb`가 **4.4.0**으로 새 패치 릴리스됨(`chdb-core`는
> 26.7.3으로 동일). 아래 벤치마크 수치는 4.3.0 기준이며 4.4.0으로 재측정하지는
> 않았습니다 — `chdb-core`(실제 엔진)가 그대로라 결과가 크게 달라질 가능성은
> 낮지만, 정밀한 재현이 필요하면 버전을 맞추고 다시 실행하세요.
