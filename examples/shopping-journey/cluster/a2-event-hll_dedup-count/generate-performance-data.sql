-- A2 성능 비교 데이터를 한 chunk씩 생성한다.
-- run-performance-data.sh가 아래 파라미터를 전달한다.
--
--   journey_offset: 전체 1천만 범위에서 이번 chunk의 시작 번호
--   journey_count:  이번 chunk의 journey 수. 현재 기준은 1,000,000 이하
...
-- 발생 시각은 빠르지만 received_at이 늦으므로 대표 event가 바뀌지 않아야 한다.
INSERT INTO shop_a2.shopping_events
    (message_id, mall_id, store_id, product_id, journey_id,
     customer_group_ids, event_kind, source_kind, occurred_at, received_at)
SELECT
